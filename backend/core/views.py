...
class JobPostingViewSet(viewsets.ModelViewSet):
    """
    Public: list & retrieve active postings.
    Admin:  full CRUD.
    """
    ...
    def get_queryset(self):
        ...
        if user and user.is_authenticated and hasattr(user, 'role') and user.role == 'admin':
...
class JobApplicationViewSet(viewsets.ModelViewSet):
    ...
    """
    filter_backends = [filters.SearchFilter]
    search_fields   = ['full_name', 'email', 'job__title', 'reference']
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {
                'message': (
                    'Thank you for your application! Our HR team will review it and be in touch. '
                    'You can track your application using the reference below.'
                ),
    ...
    @action(detail=False, methods=['get'])
    def track(self, request):
        """Public endpoint: applicant checks their own application by reference"""
        ref = request.query_params.get('reference')
        if not ref:
            return Response({'error': 'Reference required.'}, status=400)
        try:
            app = JobApplication.objects.select_related('job').get(reference=ref)
            return Response({
